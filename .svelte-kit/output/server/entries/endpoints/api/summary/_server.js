...
import { r as readData } from "../../../../chunks/dataStore.js";
const CATEGORIES = [
  "mood",
  "energy",
  "stress",
...
async function GET({ url }) {
  ...
  const userResponse = Object.fromEntries(
    CATEGORIES.map((key) => [key, url.searchParams.get(key) ?? void 0])
  );
  let exactMatches = 0;
  ...
  const message = exactMatches > 0 ? `${exactMatches} ${exactMatches === 1 ? "person feels" : "people feel"} exactly like you today!` : partialMatches > 0 ? `${partialMatches} ${partialMatches === 1 ? "person feels" : "people feel"} very similar to you today!` : "You're unique today! No one feels exactly like you.";
  return json({
    total,
    exactMatches,
    partialMatches,
